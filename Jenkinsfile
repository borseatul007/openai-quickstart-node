    ...
    agent {
        label 'abhi2'
          ...
          steps {
            ...
            sh 'sudo scp -r ./pages user@15.207.109.102:/var/www/html' // Use SCP to deploy the code to the remote server

            sh 'ssh user@15.207.109.102 pm2 start /var/www/html/pages/index.js --name my-app' // Start the Node.js application with PM2 on the remote server
